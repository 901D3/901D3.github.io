function bayer(f) {
  let
    {width: w,height: h,data: d}= f,
    bM = parseMat(gIdV("errDiffsMatInput")),
    div = pFl(gIdV("errDiffsDivInput")),
    lvls = ["r", "g", "b"].map(c => pFl(gIdV(c + "LvlsInput")) - 1),
    nM = bM.length,
    t = 255 / matSz,
    rnd = gId("randMat").checked;

  //if the matrix is 2d, make it 3d
  if (!Array.isArray(bM[0][0])) bM = [bM];

  if (gId("autoDiv").checked) {
    div = 0;
    matSz = findHighest(bM);
  } else {
    div = pFl(gIdV("errDiffsDivInput"));
  }
  ...
  for (let c = 0; c < 3; c++) {
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        let sM;
        if (rnd) {
          sM = bM[floor(random() * nM)];
        } else {
          sM = bM[0];
        }

        let mX = sM[0].length, mY = sM.length;
        let bVal = (sM[y % mY][x % mX] * t) / 255;

        d[i + c] = round((lvls[c] * (d[i + c] / 255) + bVal) / lvls[c]) * 255;
      }
    }
  }
}
...
function findHighest(matrix) {
  ...
  for (let i = 0; i < matrix.length; i++) {
    let v = matrix[i];
    if (Array.isArray(v)) {
      let nestedHighest = findHighest(v);
      if (nestedHighest > highestValue) {
        highestValue = nestedHighest;
      }
    } else if (v !== null && v == v) {
      if (v > highestValue) {
        highestValue = v;
...
function parseMat(input) {
  ...
  return blocks.map(block => {
    const rows = block.trim().split("/").map(row =>
...
function parseMatRev(input) {
  ...
  const is3D = Array.isArray(matrix) && Array.isArray(matrix[0]) && Array.isArray(matrix[0][0]);

  if (is3D) {
    return matrix.map(block => {
      const rows = block.map(row =>
        row.map(cell => {
          const trimmed = cell === "-" ? null : cell;
      ...
      const formattedRows = rows.map(row => row.map(cell => cell === null ? "-" : cell.toString()).join(", ")).join(" / ");
